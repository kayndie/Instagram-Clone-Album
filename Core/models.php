...
function insertNewAlbum($pdo, $album_name, $username) {
    $response = array();
...
function updateAlbum($pdo, $album_id, $album_name) {
    ...
    if ($stmt->execute([$album_name, $album_id])) {
        $response = array(
            "status" => "200",
...
function insertPhotoToAlbum($pdo, $photo_name, $username, $description, $album_id) {
    $response = array();
    ...
    if ($stmt->execute([$photo_name, $username, $description, $album_id])) {
        $response = array(
            "status" => "200",
            "message" => "Photo successfully uploaded to album!"
        );
    } else {
        $response = array(
            "status" => "400",
